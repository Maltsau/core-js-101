...
function getPolynom(...args) {
  return (val) => {
    ...
    for (let i = 0; i < args.length; i += 1) {
      res += args[i] * val ** (args.length - 1 - i);
    }
    return res;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map((value) => JSON.stringify(value))}) starts`);
    const resFn = func(...args);
    logFunc(`${func.name}(${args.map((value) => JSON.stringify(value))}) ends`);
    return resFn;
  };
}
