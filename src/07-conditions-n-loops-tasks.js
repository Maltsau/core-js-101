...
function findFirstSingleChar(str) {
  return [...str].find((smbl, i, st) => st.indexOf(smbl) === st.lastIndexOf(smbl)) || null;
}
...
function isCreditCardNumber(ccn) {
  const ccnArr = Array.from(String(ccn), Number);
  for (let i = ccnArr.length - 2; i >= 0; i -= 2) {
    if (ccnArr[i] * 2 > 9) {
      ccnArr[i] = ccnArr[i] * 2 - 9;
    } else ccnArr[i] *= 2;
  }
  return (ccnArr.reduce((acc, cur) => acc + cur, 0) % 10) === 0;
}
...
function getDigitalRoot(num) {
  const numSum = Array.from(String(num), Number).reduce((acc, cur) => acc + cur, 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  const pathesArrays = pathes.map((path) => path.split('/'));
  const result = pathesArrays[0].filter((dir, i) => pathesArrays.every((arr) => arr[i] === dir));
  return `${result.join('/')}/`;
}
